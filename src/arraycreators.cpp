...
#include "screen.h"             // for image array and structures h7image
...
uint8_t CreateArraytoArray(h7image image,uint32_t colortomode,bool transparant,uint32_t alphareplace)
{
  ...
  switch (colortomode){ 
      ...
      case COLORMODE_RGB565   : Create565array32(image,  alphareplace, CFile) ; break; // no transparancy, all opaque
      default : break;
  }
  fclose(CFile);
...
void Create565array32(h7image image, uint32_t alpha0,FILE *CFile)
{
uint32_t *p = (uint32_t *) image.imagearray;  
fprintf(CFile,"#define %s %i\n",image.namesy, image.height); 
fprintf(CFile,"#define %s %i\n\n",image.namesx, image.width); 
...
fprintf(CFile,"const uint16_t %s[%s][%s] = {\n",image.namea,image.namesy,image.namesx); 
      for(int t=0;t<image.height;++t) {
        for(int i=0;i<image.width;++i) {
            ...
            case  COLORMODE_ARGB8888 :  fprintf(CFile,"0x%04x,",   Convert8888to565( (uint32_t) *(p+i+t*image.width),false,alpha0)  );break;  // Convert ARGB8888 to RGB565
            case  COLORMODE_ABGR8888 :  fprintf(CFile,"0x%04x,",   Convert8888to565( (uint32_t) *(p+i+t*image.width),true,alpha0)  );break;  // Convert ABGR8888 to RGB565 swopping RB
            case  COLORMODE_XRGB8888 :  fprintf(CFile,"0x%04x,",   Convert8888to565( (uint32_t) *(p+i+t*image.width),false,alpha0)  );break;  // Convert XRGB8888 to to RGB565
            case  COLORMODE_XBGR8888 :  fprintf(CFile,"0x%04x,",   Convert8888to565( (uint32_t) *(p+i+t*image.width),true,alpha0)  );break;  // Convert XBGR8888 to to RGB565 swopping RB           
            default : break;         
            }          
          }
        fprintf(CFile,"\n");  
